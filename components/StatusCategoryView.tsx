...
const StatusGroup: React.FC<{ title: string; statuses: Record<string, string> }> = ({ title, statuses }) => (
    <div>
        <h4 className="text-base font-semibold text-stone-800 dark:text-stone-200 mb-3">{title}</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            {Object.values(statuses).map((status: string) => (
                <div key={status} className="flex items-center space-x-3 rounded-lg bg-stone-50 dark:bg-stone-800 p-3 border border-stone-200 dark:border-stone-700">
                    <Badge status={status} size="sm" />
                    <span className="text-sm text-stone-700 dark:text-stone-300">{translate(status)}</span>
                </div>
            ))}
        </div>
    </div>
