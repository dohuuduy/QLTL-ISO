...
const UpcomingAudits: React.FC<UpcomingAuditsProps> = ({ schedules, nhanSuMap, onNavigate }) => {
    ...
    if (upcomingSchedules.length === 0) {
        return (
            <Card>
                <Card.Header>
                    <h3 className="text-base font-semibold text-gray-900">Lịch Audit Sắp tới</h3>
                </Card.Header>
                <Card.Body>
                    <div className="text-center py-6">
                        <Icon type="calendar" className="mx-auto h-10 w-10 text-gray-400" />
                        <p className="mt-2 text-sm text-gray-500">Không có cuộc audit nào sắp diễn ra.</p>
                    </div>
                </Card.Body>
            </Card>
        );
                    ...
                    {upcomingSchedules.map(audit => (
                        <li key={audit.id} className="p-4 hover:bg-slate-50">
                            <div className="flex items-center justify-between">
                                <p className="text-sm font-medium text-gray-900 truncate pr-2">{audit.ten_cuoc_audit}</p>
