...
const DocumentManagementPage: React.FC<DocumentManagementPageProps> = ({ allData, onUpdateData, currentUser, onViewDetails, onToggleBookmark, initialFilter }) => {
    ...
    const getSortableHeader = (label: string, key: string) => {
        ...
        return (
            ...
            >
                ...
                <Icon 
                    type={sortIcon} 
                    className={`h-4 w-4 transition-opacity ${isSorting ? 'opacity-100 text-zinc-700' : 'opacity-0 text-zinc-400 group-hover:opacity-100'}`} 
                />
            </button>
        );
    };
    ...
    const tableColumns = [
        { 
            header: <Icon type="star" className="h-5 w-5 text-zinc-500 mx-auto" />,
            accessor: (item: DanhMucTaiLieu) => (
                ...
                >
                    <Icon 
                        ...
                        className={`h-5 w-5 ${item.is_bookmarked ? 'text-yellow-400' : 'text-zinc-300 hover:text-yellow-400'}`} 
                    />
                </button>
            ),
            className: 'w-12'
        ...
        { header: getSortableHeader('Tên tài liệu', 'ten_tai_lieu'), accessor: (item: DanhMucTaiLieu) => item.ten_tai_lieu, className: 'font-medium text-zinc-900' },
                ...
                if (item.file_pdf) {
                    return (
                        <a
                            ...
                            className="inline-flex items-center justify-center text-zinc-500 hover:text-teal-700 w-full"
                            title="Mở PDF để in"
                        >
                 ...
                 return (
                    <span className="inline-flex items-center justify-center text-zinc-300 w-full cursor-not-allowed" title="Không có file PDF">
                        <Icon type="printer" className="h-5 w-5" />
                    </span>
                );
            },
            ...
            <div className="space-y-6 no-print">
                <div className="sm:flex sm:items-center sm:justify-between">
                    <div className="flex-1">
                        <h1 className="text-3xl font-bold text-zinc-900">Quản lý Tài liệu</h1>
                        ...
                        <div className="flex flex-wrap items-center gap-4 mb-4">
                            <div className="relative flex-grow min-w-[200px]">
                                <input
                                    ...
                                    className="block w-full rounded-md border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500 sm:text-sm dark:placeholder-zinc-400"
                                />
                            </div>
                            <select
                                value={filters.status}
                                onChange={(e) => setFilters(f => ({ ...f, status: e.target.value as DocumentStatus | '' | 'all_docs' }))}
                                className="block w-full flex-grow min-w-[180px] sm:w-auto rounded-md border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500 sm:text-sm"
                            >
                            ...
                            <button
                                ...
                                className={`inline-flex items-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold shadow-sm ring-1 ring-inset ${
                                    showBookmarkedOnly
                                        ...
                                        : 'bg-white text-zinc-900 ring-zinc-300 hover:bg-zinc-50'
                                }`}
                            >
                        ...
                        >
                            <div className="flex items-center gap-2 text-sm text-zinc-700">
                                ...
                                <select
                                    ...
                                    className="rounded-md border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-200 shadow-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500 sm:text-sm py-1"
                                >
